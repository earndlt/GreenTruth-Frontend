...
import { userUsageData } from '../data/mockData';

interface UserUsageTabProps {
  selectedDate?: Date;
}

const UserUsageTab: React.FC<UserUsageTabProps> = ({ selectedDate = new Date() }) => {
  // In a real implementation, we would fetch data for the selected month
  // const usageData = fetchUserUsageData(selectedDate);
  ...
  const columns = [
    {
      key: 'userName',
      header: 'User',
      cell: (item: typeof usageData[0]) => item.userName,
      align: 'left' as const
    },
    {
      key: 'email',
      header: 'Email',
      cell: (item: typeof usageData[0]) => item.email,
      align: 'left' as const
    },
    {
      key: 'wordsReadUsed',
